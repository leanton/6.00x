...
class Patient(object):
    ...
    def update(self):
        ...
        for virus in self.viruses:
            tempvir =  virus.reproduce(density)
            if tempvir != None and len(temp) < self.maxPop:
                temp.append(tempvir)
        self.viruses = temp[:]
        return len(self.viruses)
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        for virus in self.viruses:
            tempvir =  virus.reproduce(density, self.getPrescriptions())
            if tempvir != None and len(temp) < self.maxPop:
                temp.append(tempvir)
        self.viruses = temp[:]
        return len(self.viruses)
...
simulationWithDrug(75, 100, .8, 0.1, {"guttagonol": False}, 0.8, 500)

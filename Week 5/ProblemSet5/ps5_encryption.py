...
def buildCoder(shift):
    ...
    for i in range(len(str_up)):
        cipher_dict[str_up[i]] = str_up[(i+shift)%len(str_up)]
    for i in range(len(str_low)):
        cipher_dict[str_low[i]] = str_low[(i+shift)%len(str_low)]
    return cipher_dict
...
def findBestShift(wordList, text):
    ...
    for shift in range(0, 26):
        count = 0
        real_text = applyShift(text, shift)
        words = real_text.split(' ')
        #print words
        for word in words:
            if isWord(wordList, word):
                count += 1
        shift_dict[count] = shift
    return shift_dict[max(shift_dict.keys())]
